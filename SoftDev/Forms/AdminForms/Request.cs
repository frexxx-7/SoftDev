...
namespace SoftDev.Forms.AdminForms
{
    public partial class Request : Form
    {
        ...
        private void loadInfoRequests()
        {
            ...
            string query = $"select requests.id, requests.dateRequest, requests.name, requests.formularProblem, software.name, organizations.name from requests " +
        ...
        {
            DB db = new DB();
            MySqlCommand command = new MySqlCommand($"INSERT into requests (dateRequest, name, formularProblem, idSoftware, idOrganizations) values(@dateRequest, @name, @formularProblem, @idSoftware, @idOrganizations)", db.getConnection());
        ...
        {
            DB db = new DB();
            MySqlCommand command = new MySqlCommand($"update requests set dateRequest=@dateRequest, name=@name, formularProblem=@formularProblem, idSoftware=@idSoftware, idOrganizations=@idOrganizations where id = {idRegion}", db.getConnection());
            command.Parameters.AddWithValue("@dateRequest", DateRequestDateTimePicker.Value.ToString("yyyy.MM.dd"));
            command.Parameters.AddWithValue("@name", NameTextBox.Text);
            command.Parameters.AddWithValue("@formularProblem", FormularProblemTextBox.Text);
            command.Parameters.AddWithValue("@idSoftware", (SoftWareComboBox.SelectedItem as ComboBoxItem).Value);
